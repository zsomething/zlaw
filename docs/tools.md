...
| `read_file` | Reads a file; supports `offset` and `limit` for large files |
...
| `list_cronjobs` | Lists all scheduled cron jobs |
| `create_cronjob` | Creates a new cron job |
| `delete_cronjob` | Removes a cron job by ID |
...
```toml
[tools]
allowed          = ["read_file", "bash", "memory_save", "memory_recall"]
