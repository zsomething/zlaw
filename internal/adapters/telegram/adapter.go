...
func (a *Adapter) handleMessage(ctx context.Context, msg *TGMsg) {
	...
	if strings.HasPrefix(text, "/") {
		...
		if matched {
			// /clear archives the old session file, so the next regular message
			// will start a fresh log under the same deterministic session ID.
			// Drop the stale chatSink so a new one is registered for the fresh session.
			if strings.HasPrefix(cmdText, "/clear") {
				a.mu.Lock()
